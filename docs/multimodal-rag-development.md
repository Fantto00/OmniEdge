...
通过真机 POC 后，用户可选择“Add Audio to Knowledge Base”。应用会取得该 SAF URI 的只读持久授权，转写后仅对用于索引的文本规范化中文词间空格（原始音频不复制或持久化），再复用 `ContentIngestionUseCase` 完成切分、嵌入与 `Document + Chunk` 单事务写入。转写、嵌入或取消发生在写入前时不会留下孤儿记录；文档列表中显示的是可检索的转写文本。格式支持仍以真实设备验证为准。

Vosk 运行时会引入原生 `libvosk.so`（`arm64-v8a`、`armeabi-v7a`、`x86`、`x86_64`）以及 JNA 的 `libjnidispatch.so`；当前 debug APK 实测为 352,568,441 bytes。相对图片 OCR 阶段记录的 311,607,048 bytes 调试包，Vosk 与音频入库功能累计增加 40,961,393 bytes；其中本次音频入库代码较上次 Vosk POC 基线增加 32,768 bytes，未新增依赖或 ABI。该数值只用于当前构建配置的回归基线。移除方式是删除 `vosk-android` 依赖和本 POC 源码；已下载模型则位于应用的 `noBackupFilesDir`，清除应用数据即可移除。
...
1. 合并文档、图片、音频的内容列表，显示来源类型、状态和失败重试。当前第一切片复用既有内容列表：已完成的项目显示 `Document`、`Image OCR` 或 `Audio transcript` 及 `Indexed`；进行中和失败状态仍显示在各导入任务区域，尚未持久化为可跨进程恢复的任务记录。
